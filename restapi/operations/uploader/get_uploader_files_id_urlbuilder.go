...
func (o *GetUploaderFilesIDURL) Build() (*url.URL, error) {
	...
	var _path = "/uploader/files/{id}"

	id := swag.FormatInt64(o.ID)
	if id != "" {
		_path = strings.Replace(_path, "{id}", id, -1)
	} else {
		return nil, errors.New("id is required on GetUploaderFilesIDURL")
	}
...
func (o *GetUploaderFilesIDURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetUploaderFilesIDURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetUploaderFilesIDURL")
	}
